package com.cid.adsSymbols.building.columns.percent.bottom

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.PathFillType
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import com.cid.adsSymbols.foundation.IdsSymbols

val IdsSymbols.Filled.BuildingColumnsPercentBottom: ImageVector
    get() {
        if (buildingColumnsPercentBottom != null) {
            return buildingColumnsPercentBottom!!
        }
        buildingColumnsPercentBottom = ImageVector.Builder(
            name = "building-columns-percent-bottom-filled",
            defaultWidth = 24.dp,
            defaultHeight = 24.dp,
            viewportWidth = 24f,
            viewportHeight = 24f
        ).apply {
            path(fill = SolidColor(Color(0xFF000000))) {
                moveTo(19.462f, 9f)
                horizontalLineTo(4.538f)
                curveTo(3.756f, 9f, 3.438f, 7.993f, 4.079f, 7.545f)
                lineTo(11.541f, 2.321f)
                curveTo(11.817f, 2.129f, 12.183f, 2.129f, 12.459f, 2.321f)
                lineTo(19.921f, 7.545f)
                curveTo(20.562f, 7.993f, 20.244f, 9f, 19.462f, 9f)
                close()
            }
            path(fill = SolidColor(Color(0xFF000000))) {
                moveTo(2f, 21f)
                curveTo(2f, 20.448f, 2.448f, 20f, 3f, 20f)
                horizontalLineTo(12.362f)
                curveTo(12.635f, 20.748f, 13.052f, 21.426f, 13.58f, 22f)
                horizontalLineTo(3f)
                curveTo(2.448f, 22f, 2f, 21.552f, 2f, 21f)
                close()
            }
            path(fill = SolidColor(Color(0xFF000000))) {
                moveTo(12f, 17.943f)
                curveTo(12f, 17.622f, 12.025f, 17.307f, 12.074f, 17f)
                horizontalLineTo(5f)
                curveTo(4.448f, 17f, 4f, 17.448f, 4f, 18f)
                curveTo(4f, 18.552f, 4.448f, 19f, 5f, 19f)
                horizontalLineTo(12.093f)
                curveTo(12.032f, 18.657f, 12f, 18.303f, 12f, 17.943f)
                close()
            }
            path(fill = SolidColor(Color(0xFF000000))) {
                moveTo(13f, 14.625f)
                curveTo(13.512f, 13.855f, 14.197f, 13.21f, 15f, 12.745f)
                verticalLineTo(11f)
                curveTo(15f, 10.448f, 14.552f, 10f, 14f, 10f)
                curveTo(13.448f, 10f, 13f, 10.448f, 13f, 11f)
                verticalLineTo(14.625f)
                close()
            }
            path(fill = SolidColor(Color(0xFF000000))) {
                moveTo(18f, 11.943f)
                curveTo(18.341f, 11.943f, 18.675f, 11.971f, 19f, 12.026f)
                verticalLineTo(11f)
                curveTo(19f, 10.448f, 18.552f, 10f, 18f, 10f)
                curveTo(17.448f, 10f, 17f, 10.448f, 17f, 11f)
                verticalLineTo(12.026f)
                curveTo(17.325f, 11.971f, 17.659f, 11.943f, 18f, 11.943f)
                close()
            }
            path(fill = SolidColor(Color(0xFF000000))) {
                moveTo(6f, 10f)
                curveTo(6.552f, 10f, 7f, 10.448f, 7f, 11f)
                verticalLineTo(15f)
                curveTo(7f, 15.552f, 6.552f, 16f, 6f, 16f)
                curveTo(5.448f, 16f, 5f, 15.552f, 5f, 15f)
                verticalLineTo(11f)
                curveTo(5f, 10.448f, 5.448f, 10f, 6f, 10f)
                close()
            }
            path(fill = SolidColor(Color(0xFF000000))) {
                moveTo(11f, 11f)
                curveTo(11f, 10.448f, 10.552f, 10f, 10f, 10f)
                curveTo(9.448f, 10f, 9f, 10.448f, 9f, 11f)
                verticalLineTo(15f)
                curveTo(9f, 15.552f, 9.448f, 16f, 10f, 16f)
                curveTo(10.552f, 16f, 11f, 15.552f, 11f, 15f)
                verticalLineTo(11f)
                close()
            }
            path(fill = SolidColor(Color(0xFF000000))) {
                moveTo(19.854f, 16.147f)
                curveTo(20.049f, 16.342f, 20.049f, 16.659f, 19.854f, 16.854f)
                lineTo(16.854f, 19.854f)
                curveTo(16.658f, 20.049f, 16.342f, 20.049f, 16.146f, 19.854f)
                curveTo(15.951f, 19.659f, 15.951f, 19.342f, 16.146f, 19.147f)
                lineTo(19.146f, 16.147f)
                curveTo(19.342f, 15.951f, 19.658f, 15.951f, 19.854f, 16.147f)
                close()
            }
            path(fill = SolidColor(Color(0xFF000000))) {
                moveTo(16.75f, 17.5f)
                curveTo(17.164f, 17.5f, 17.5f, 17.164f, 17.5f, 16.75f)
                curveTo(17.5f, 16.336f, 17.164f, 16f, 16.75f, 16f)
                curveTo(16.336f, 16f, 16f, 16.336f, 16f, 16.75f)
                curveTo(16f, 17.164f, 16.336f, 17.5f, 16.75f, 17.5f)
                close()
            }
            path(fill = SolidColor(Color(0xFF000000))) {
                moveTo(20f, 19.25f)
                curveTo(20f, 19.664f, 19.664f, 20f, 19.25f, 20f)
                curveTo(18.836f, 20f, 18.5f, 19.664f, 18.5f, 19.25f)
                curveTo(18.5f, 18.836f, 18.836f, 18.5f, 19.25f, 18.5f)
                curveTo(19.664f, 18.5f, 20f, 18.836f, 20f, 19.25f)
                close()
            }
            path(
                fill = SolidColor(Color(0xFF000000)),
                pathFillType = PathFillType.EvenOdd
            ) {
                moveTo(13.25f, 18f)
                curveTo(13.25f, 15.377f, 15.377f, 13.25f, 18f, 13.25f)
                curveTo(20.623f, 13.25f, 22.75f, 15.377f, 22.75f, 18f)
                curveTo(22.75f, 20.624f, 20.623f, 22.75f, 18f, 22.75f)
                curveTo(15.377f, 22.75f, 13.25f, 20.624f, 13.25f, 18f)
                close()
                moveTo(18f, 14.75f)
                curveTo(16.205f, 14.75f, 14.75f, 16.205f, 14.75f, 18f)
                curveTo(14.75f, 19.795f, 16.205f, 21.25f, 18f, 21.25f)
                curveTo(19.795f, 21.25f, 21.25f, 19.795f, 21.25f, 18f)
                curveTo(21.25f, 16.205f, 19.795f, 14.75f, 18f, 14.75f)
                close()
            }
        }.build()

        return buildingColumnsPercentBottom!!
    }

private var buildingColumnsPercentBottom: ImageVector? = null